...
if($imgimageFileType != "jpg" && $imgimageFileType != "jpeg" && $imgimageFileType != "png") {
    header("Location: /co_curricular/admin/iic.php?err=Sorry, Image only jpg or jpeg or png files are allowed.");
    die();
}
...
{
header("Location: /co_curricular/admin/iic.php?err=Image size should be 300px X 200px.");
die();
}

  if (move_uploaded_file($_FILES["img"]["tmp_name"], $imgtarget_file)) {
      $sql = "INSERT INTO club (file,name)
      VALUES ('$imgfile_name','iic')";
  
      if ($conn->query($sql) === TRUE) {
          header("Location: /co_curricular/admin/iic.php?page=1&msg=Img added Successfully !");
          die();
      } else {
          echo "Error: " . $sql . "<br>";
      }
      ...
      header("Location: /co_curricular/admin/iic.php?page=1&err=Sorry, there was an error uploading your image.");
      die();
  }
